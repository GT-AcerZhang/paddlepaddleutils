...
def transformer(src_vocab_size,
    ...
    # using masked lm Layer form yukun
    mask_id = 0
    src_word = enc_inputs[0]
    logging.debug("inputs of mask_lm:{0}".format(src_word))
    layers.Print(src_word)
    src_word, mask = layers.mask_lm(src_word, mask_id, src_vocab_size, 0.15, seed=1)
    logging.debug("src_word after mask_lm:{0}".format(src_word))
    layers.Print(src_word)
    logging.debug("mask after mask_lm:{0}".format(mask))
    layers.Print(mask)
    ...
    predict = wrap_decoder(
        ...
        enc_output, )
    '''
    fc_out = layers.fc(input=enc_output, size=src_vocab_size, num_flatten_dims=2, act=None)
    fc_out = layers.reshape(fc_out, shape=[-1, src_vocab_size])

    mask = layers.reshape(mask, shape=[-1, 1])
    
    cost = fluid.layers.softmax_with_cross_entropy(logits = fc_out, 
                                                   label = mask, 
                                                   ignore_index = -1,
                                                   soft_label = False)
    ...
    return sum_cost, avg_cost, fc_out, token_num, reader if use_py_reader else None
